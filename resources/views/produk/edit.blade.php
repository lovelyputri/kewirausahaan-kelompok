...
<style>
    ...
    .produk-form-page .info-card { background: #faf7f3; border: 1px solid #f0e8de; border-radius: 12px; padding: 12px 16px; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; font-family: "Inter", sans-serif; font-size: 12.5px; color: #6b4d38; }
    .produk-form-page .info-card i { width: 16px; height: 16px; color: #a89a8c; flex-shrink: 0; }
    .produk-form-page .info-card strong { font-weight: 700; color: #3f3025; }
        ...
        <form method="POST" action="{{ route('produk.update', $produk->id) }}">
                ...
                <div class="dropdown" data-dropdown>
                    ...
                    <button type="button" class="dropdown-toggle" onclick="toggleDropdown(this)">
                        ...
                        <span class="dropdown-label {{ old('id_kategori', $produk->id_kategori) ? '' : 'placeholder' }}">
                            @php $selectedKat = $kategoris->firstWhere('id', old('id_kategori', $produk->id_kategori)); @endphp
            ...
            <div class="form-group">
                ...
                <div class="input-wrap">
                    ...
                    <select name="gambar" id="gambarSelect"
                        ...
                        @foreach($gambarList as $gbr)
                            <option value="{{ $gbr }}" {{ old('gambar', $produk->gambar) == $gbr ? 'selected' : '' }}>
                                {{ $gbr }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="gambar-preview" id="gambarPreview">
                    @if($produk->gambar)
                        <img src="{{ asset('images/' . $produk->gambar) }}" alt="Preview">
                    @else
                        <span>Preview</span>
                    @endif
                </div>
